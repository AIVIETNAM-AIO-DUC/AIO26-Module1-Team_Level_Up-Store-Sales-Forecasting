...
- **"Log space" nghĩa là gì:** "space" ở đây chỉ là cách gọi tắt cho *tập số bạn đang làm việc
  ngay lúc này*. Pipeline có hai:

  | Space      | Giá trị        | Ví dụ (sales = 0, 1, 50, 1000)     |
  |------------|----------------|------------------------------------|
  | Raw space  | `y` (sales)    | `0, 1, 50, 1000`                   |
  | Log space  | `log1p(y)`     | `0.000, 0.693, 3.932, 6.908`       |

  `log1p` là cửa đi vào, `expm1` là cửa đi ra. Trong log space, bạn fit model, sinh prediction,
  và tính loss; `expm1` đưa bạn về sales thật ở bước cuối.
